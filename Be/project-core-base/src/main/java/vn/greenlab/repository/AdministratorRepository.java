...
public interface AdministratorRepository extends BaseRepository<Administrator, Integer> {
  ...
  @Query("SELECT a FROM Administrator a WHERE a.status = 1 AND (:keyword IS NULL OR :keyword = '' OR LOWER(a.full_name) LIKE LOWER(CONCAT('%', :keyword, '%')) OR LOWER(a.user_name) LIKE LOWER(CONCAT('%', :keyword, '%')) OR LOWER(a.email) LIKE LOWER(CONCAT('%', :keyword, '%')) OR a.phone LIKE CONCAT('%', :keyword, '%')) AND (:position IS NULL OR :position = :position)")
  List<Administrator> findByKeywordAndPosition(@Param("keyword") String keyword, @Param("position") Position position);

  Optional<Administrator> findByProviderAndProviderId(SocialProvider provider, String providerId);
