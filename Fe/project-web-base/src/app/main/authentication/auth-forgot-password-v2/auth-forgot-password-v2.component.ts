...
export class AuthForgotPasswordV2Component implements OnInit {
  ...
  forgotPassword() {
    this._authenticationService.forgotPassword(this.forgotPasswordForm.value.email).subscribe(res => {
      if (res.success) {
        this._toastr.success('Email hướng dẫn đặt lại mật khẩu đã được gửi đi. Vui lòng kiểm tra email để đặt lại mật khẩu');
      } 
    });
  }
