...
import { UnitService, Unit } from '../../../services/lab-tests/unit.service';
...
export class UnitsListComponent extends BaseComponent implements OnInit, OnDestroy {
  public rows: Unit[] = [];
  ...
  public selectedItem: Unit | null = null;
  ...
  public newItem: Partial<Unit> = { code: '', name: '', symbol: '', isActive: true };
  ...
  async ngOnInit(): Promise<void> {
    this.titleService.setTitle('Quản lý đơn vị');
  ...
  setColumn() {
    this.columns = [
      ...
      { name: 'Tên đơn vị', key: 'name', width: '30%', cellTemplate: this.nameTemplate },
      { name: 'Ký hiệu', key: 'symbol', width: '15%' },
  ...
  openModal(item: Unit, mode: 'view' | 'edit') { this.selectedItem = { ...item }; this.modalMode = mode; this.showModal = true; }
  closeModal() { this.showModal = false; this.selectedItem = null; }
  openModalCreate() { this.newItem = { code: '', name: '', symbol: '', isActive: true }; this.showCreateModal = true; }
  closeModalCreate() { this.showCreateModal = false; }
  openModalDelete(item: Unit) { this.selectedItem = { ...item }; this.showDeleteModal = true; }
  closeModalDelete() { this.showDeleteModal = false; this.selectedItem = null; }

  viewItem(item: Unit) { this.openModal(item, 'view'); }
  editItem(item: Unit) { this.openModal(item, 'edit'); }

  async saveItem() {
    try {
      this.updating = true;
      const result = await this.service.updateUnit(this.selectedItem!);
  ...
  async createItem() {
    try {
      this.creating = true;
      const result = await this.service.createUnit(this.newItem);
  ...
  async deleteItem() {
    try {
      this.deleting = true;
      const result = await this.service.deleteUnit(this.selectedItem!.id!);
