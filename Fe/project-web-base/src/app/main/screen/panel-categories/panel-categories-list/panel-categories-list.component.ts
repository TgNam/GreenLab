...
import { PanelCategoryService, PanelCategory } from '../../../services/lab-tests/panel-category.service';
import { Title } from '@angular/platform-browser';

@Component({
  selector: 'app-panel-categories-list',
  templateUrl: './panel-categories-list.component.html',
  styleUrls: ['./panel-categories-list.component.scss']
})
export class PanelCategoriesListComponent extends BaseComponent implements OnInit, OnDestroy {
  public rows: PanelCategory[] = [];
  ...
  public selectedItem: PanelCategory | null = null;
  ...
  public newItem: Partial<PanelCategory> = { code: '', name: '', description: '', isActive: true };
  ...
  constructor(
    ...
    private service: PanelCategoryService
  ) {
  ...
  async ngOnInit(): Promise<void> {
    this.titleService.setTitle('Quản lý danh mục gói');
  ...
  setColumn() {
    this.columns = [
      ...
      { name: 'Tên danh mục gói', key: 'name', width: '30%', cellTemplate: this.nameTemplate },
      { name: 'Trạng thái', key: 'isActive', width: '15%', cellTemplate: this.statusTemplate },
      { name: 'Thời gian', key: 'createdAt', width: '20%', cellTemplate: this.createdAtTemplate },
      { name: 'Hành động', key: 'action', width: '20%', cellTemplate: this.actionTemplate },
    ];
  ...
  openModal(item: PanelCategory, mode: 'view' | 'edit') { this.selectedItem = { ...item }; this.modalMode = mode; this.showModal = true; }
  closeModal() { this.showModal = false; this.selectedItem = null; }
  openModalCreate() { this.newItem = { code: '', name: '', description: '', isActive: true }; this.showCreateModal = true; }
  closeModalCreate() { this.showCreateModal = false; }
  openModalDelete(item: PanelCategory) { this.selectedItem = { ...item }; this.showDeleteModal = true; }
  closeModalDelete() { this.showDeleteModal = false; this.selectedItem = null; }

  viewItem(item: PanelCategory) { this.openModal(item, 'view'); }
  editItem(item: PanelCategory) { this.openModal(item, 'edit'); }

  async saveItem() {
    try {
      this.updating = true;
      const result = await this.service.updatePanelCategory(this.selectedItem!);
  ...
  async createItem() {
    try {
      this.creating = true;
      const result = await this.service.createPanelCategory(this.newItem);
  ...
  async deleteItem() {
    try {
      this.deleting = true;
      const result = await this.service.deletePanelCategory(this.selectedItem!.id!);
