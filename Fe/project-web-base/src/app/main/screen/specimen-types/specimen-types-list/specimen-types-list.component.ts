...
import { SpecimenTypeService, SpecimenType } from '../../../services/lab-tests/specimen-type.service';
import { Title } from '@angular/platform-browser';

@Component({
  selector: 'app-specimen-types-list',
  templateUrl: './specimen-types-list.component.html',
  styleUrls: ['./specimen-types-list.component.scss']
})
export class SpecimenTypesListComponent extends BaseComponent implements OnInit, OnDestroy {
  public rows: SpecimenType[] = [];
  ...
  public selectedItem: SpecimenType | null = null;
  ...
  public newItem: Partial<SpecimenType> = { code: '', name: '', description: '', isActive: true };
  ...
  constructor(
    ...
    private service: SpecimenTypeService
  ) {
  ...
  async ngOnInit(): Promise<void> {
    this.titleService.setTitle('Quản lý loại mẫu');
  ...
  setColumn() {
    this.columns = [
      ...
      { name: 'Tên loại mẫu', key: 'name', width: '25%', cellTemplate: this.nameTemplate },
  ...
  openModal(item: SpecimenType, mode: 'view' | 'edit') { this.selectedItem = { ...item }; this.modalMode = mode; this.showModal = true; }
  ...
  openModalDelete(item: SpecimenType) { this.selectedItem = { ...item }; this.showDeleteModal = true; }
  closeModalDelete() { this.showDeleteModal = false; this.selectedItem = null; }

  viewItem(item: SpecimenType) { this.openModal(item, 'view'); }
  editItem(item: SpecimenType) { this.openModal(item, 'edit'); }

  async saveItem() {
    try {
      this.updating = true;
      const result = await this.service.updateSpecimenType(this.selectedItem!);
  ...
  async createItem() {
    try {
      this.creating = true;
      const result = await this.service.createSpecimenType(this.newItem);
  ...
  async deleteItem() {
    try {
      this.deleting = true;
      const result = await this.service.deleteSpecimenType(this.selectedItem!.id!);
